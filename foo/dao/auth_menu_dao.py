...
class AuthMenuDao():
    ...
    def insert(self, id,pid,lft,rgt,depth,name,path,icon,hideInMenu):
        sql = """INSERT INTO auth_menu (id,pid,lft,rgt,depth,name,path,icon,hideInMenu)
                 VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s)"""
        params = (id,pid,lft,rgt,depth,name,path,icon,hideInMenu)
        logging.debug(sql, *params)
        rowcount = MysqlUtil().insert_one(sql, params)
        return rowcount


    def update(self, id, name,path,icon,hideInMenu):
        sql = """UPDATE auth_menu
                 SET name=%s,path=%s,icon=%s,hideInMenu=%s
                 WHERE id=%s"""
        params = (name,path,icon,hideInMenu, id)
        logging.debug(sql, *params)
    ...
    def select(self, id):
        sql = """SELECT id,pid,lft,rgt,depth,name,path,icon,hideInMenu
                 FROM auth_menu
                 WHERE id=%s"""
        params = (id,)
        logging.debug(sql, *params)
